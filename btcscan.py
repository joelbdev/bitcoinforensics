...
with open('seed.txt', 'r') as f:
    seedlist = [word for line in f for word in line.split()]
    for word in seedlist:
        byteslist.append(word.encode("utf-8"))
    f.close()
...
def examine_file(file_2_examine):
    ...
    try:
        with open(file_2_examine, 'rb') as f, mmap(f.fileno(), 0, access=ACCESS_READ) as mm:
            #check for seed phrase match
            for y in range(0, len(byteslist)):
                for match in re.finditer(byteslist[y], mm):
                    matchedseed.append(match)

            if len(matchedseed) == 12: #will provide possible matches to a seed phrase when exactly 12 of the seed words has been located
                output_file.write('"' + str('possible seed match found') + '","' + file_2_examine + '","' + 's' + '","' + 'seed' + '","' + 'seed' +'"\n')
